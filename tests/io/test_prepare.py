...
from turf.io.prepare import _extract_metadata
...
def test_prepare_exit_code_zero(
    ...
    monkeypatch.setattr("turf.io.prepare.get_root", lambda: tmp_path)
    monkeypatch.setattr("turf.io.prepare._run_preprocessing", lambda *a, **kw: None)
...
def test_prepare_calls_preprocessing_with_correct_args(
    ...
    monkeypatch.setattr("turf.io.prepare._run_preprocessing", _capture)
...
def test_prepare_does_not_create_output_dir_on_failure(
    ...
    monkeypatch.setattr("turf.io.prepare._run_preprocessing", _fail)
    runner.invoke(app, ["dataset", "prepare", entry.id])
